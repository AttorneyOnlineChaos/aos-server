 ...
 */
class AOClient : public QObject, public theory::PacketTransmitter
{
  Q_OBJECT
...
public:
  ...
  struct CommandInfo
  {
    QList<ACLRole::Permission> acl_permissions; //!< The permissions necessary to be able to run the command. @see ACLRole::Permission.
   ...
   * @param f_remote_ip The resolved IP address of the client.
  ...
  theory::Shared<theory::CargoSocket> socket() const;
  void setSocket(const theory::Shared<theory::CargoSocket> &socket);

  bool processPendingPacket(const theory::Packet &packet);

  SessionStatus sessionStatus() const;
  void setSessionStatus(SessionStatus f_status);
  void markActive();
  void markInactive();
  void markExpired();
   ...
   */
  std::optional<QString> m_showname;
   ...
   */
  theory::PlayerStatus m_status = theory::PlayerStatus::Online;
   ...
   */
  QList<theory::CharacterId> m_charcurse_list;
   ...
   * @param message The text of the message to send.
   */
  void sendServerMessageArea(const QString &message);
  ...
  /**
   * @brief Adds the last send IC-Message to QList of the respective area.
   *
   * @details This one pulls double duty to both append IC-Messages to the QList or insert them, depending on the current recorder enum.
   *
   * @param packet The MS-Packet being recorded with their color changed to green.
   */
  void addStatement(theory::IcMessagePacket message);
   ...
   */
  theory::IcMessagePacket updateStatement(theory::IcMessagePacket message);
   ...
   */
  void updateJudgeLog(AreaData *area, AOClient *client, const QString &action);
  ...
  /**
   * @brief The last known offsets, on each axis.
   */
  int m_offset_x = 0;
  int m_offset_y = 0;
...
private:
  ...
  void process(const theory::PlayMusicPacket &packet);
   ...
   */
  void changePosition(const QString &new_pos);
   ...
   *
   * @details Takes a **status** as an argument.
   *
   * @iscommand
   */
   ...
   *
   * @details Using this deletes the entire entry in the QList and resizes it appropriately to prevent empty record indices.
   */
   ...
   *
   * @details Loads a titled text file which contains the edited packets to be loaded into the QList.
  ...
  /**
   * @brief Clears QList of the current area.
   *
   ...
   */
  void sendNotice(const QString &f_notice, bool f_global = false);
...
Q_SIGNALS:
  ...
  void sessionStatusChanged(SessionStatus);
};
} // namespace kenji
