...
namespace kenji
{
using BanDuration = long long;
inline constexpr BanDuration PermanentBanDuration = 3153600000;
...
struct BanInfo
{
  ...
  BanDuration duration; //!< The duration of the ban, in seconds.
  bool revoked = false; //!< The status of the ban.
...
public:
   ...
   *
   * @details Creates a database file at `config/kenji.db`, and creates two tables in it:
   ...
   */
  int getBanID(const QHostAddress &ip);
   ...
   */
  bool createUser(const QString &username, const QByteArray &salt, const QString &password, const QString &acl);
