...
void kenji::AOClient::process(const theory::ModCallPacket &packet)
{
  ...
  m_logger.logModcall(l_areaName, userId, name(), QString::number(id), (m_character.toString() + " " + characterName().value_or(QString())));
