...
inline int kenji::MedievalParser::randomInt(int min, int max)
{
  if (min > max)
  {
    return 0;
...
inline bool kenji::MedievalParser::containsCaseInsensitive(const QList<QString> &vector, const QString &str)
{
  return vector.contains(str, Qt::CaseInsensitive);
}

kenji::MedievalParser::MedievalParser()
{
  parseDataFile();
}

QString kenji::MedievalParser::degrootify(const QString &message)
{
  if (!datafile_valid)
  {
    return message;
...
void kenji::MedievalParser::parseDataFile()
{
  datafile_valid = true;
  ...
  { // Non-Terminating error.
    zWarning(log::config) << "Unable to load Medieval Mode data file. The following error occurred: " + l_error.errorString();
    datafile_valid = false;
    return;
  }
  ...
  for (const QJsonValue &replacement : l_Json_replacement_array)
  {
    ...
    for (const QString &key : rep_obj.keys())
    {
      if (key == "replacement")
      {
        replacement_struct.replacements = QList<QString>(rep_obj[key].toVariant().toStringList().toVector());
      }
      else if (key == "replacement_prepend")
      {
        replacement_struct.prepended = QList<QString>(rep_obj[key].toVariant().toStringList().toVector());
      }
      else if (key == "replacement_plural")
      {
        replacement_struct.plural_replacements = QList<QString>(rep_obj[key].toVariant().toStringList().toVector());
      }
      ...
      {
        replacement_struct.words = QList<QString>(rep_obj[key].toVariant().toStringList().toVector());
      ...
      {
        replacement_struct.plurals = QList<QString>(rep_obj[key].toVariant().toStringList().toVector());
        for (const QString &word : replacement_struct.words)
        {
          word_vector.append(word);
        }
      ...
      {
        replacement_struct.prev_words = QList<QString>(rep_obj[key].toVariant().toStringList().toVector());
        for (const QString &word : replacement_struct.words)
        {
          word_vector.append(word);
        }
...
QString kenji::MedievalParser::getRandomPre()
{
  if (randomInt(1, 4) != 1)
  {
    return "";
  ...
  prev_pre += (randomInt(1, 4));
  while (prev_pre >= prepended_words.count())
  { // ensure we do not go out of bounds
    prev_pre -= prepended_words.count();
  }

  return prepended_words[prev_pre];
}

QString kenji::MedievalParser::getRandomPost()
{
  if (randomInt(1, 5) != 1)
  {
    return "";
  ...
  prev_post += randomInt(1, 4);
  while (prev_post >= appended_words.count())
  { // ensure we do not go out of bounds
    prev_post -= appended_words.count();
  }

  return appended_words[prev_post];
}

kenji::MatchResult kenji::MedievalParser::wordMatches(WordReplacement *rep, ReplacementCheck *check)
{
  if (rep->chance != 1)
  {
...
bool kenji::MedievalParser::replaceWord(ReplacementCheck *check, QString *rep_str, bool symbols, bool word_list_only)
{
  *rep_str = "";
  ...
  for (auto &&replacement : word_replacements)
  {
    ...
    if (rep_ptr->prepended.count() > 0)
    {
      ...
      for (int count = 0; count < rep_ptr->prepend_count; count++)
      {
        ...
        if (count + 1 < rep_ptr->prepend_count)
        { // we have more prepends to prepend
          rep_str->append(", ");
        }
        else
        {
  ...
  if (!symbols && !word_list_only)
  {
    ...
    if (check->word.length() >= 3)
    {
      ...
      if (randomInt(1, 5) == 1 && (lc == 't' || lc == 'p' || lc == 'k' || lc == 'g' || lc == 'b' || lc == 'w'))
      {
        *rep_str = check->word;
        rep_str->append("eth");
        return true;
...
bool kenji::MedievalParser::performReplacement(const QString &rep_str, ReplacementCheck *check, QString stored_word, QString *out_text)
{
  if (!rep_str.isEmpty())
  {
...
QString kenji::MedievalParser::modifySpeech(QString text, bool generate_pre_and_post, bool in_pre_post)
{
  QString final_text;
